class Api::V1::ForgotPasswordController < Api::V1::ApiController
  ...
  def reset_password
    ...
    return render json: {message: "Email/Phone is blank"}  if user_params[:email].blank? && user_params[:phone_number].blank?
    ...
    if user.present? && user.password_token_valid?
      ...
      render json: { message: ['Link not valid or expired. Try again'] }, status: :not_found
    end
  end
