class DashboardsController < ApplicationController
  def index
    ...
    @new_users = User.new_users.order(created_at: :desc).paginate(page: params[:page], per_page: 10)
    ...
    @new_users = User.new_users.where("created_at >= ? AND created_at <= ?", @start_date.beginning_of_day, @end_date.end_of_day).paginate(page: params[:page], per_page: 10) if @start_date.present? && @end_date.present?   
